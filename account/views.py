...
class UserLoginView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, 'you loged successfully.', 'success')
                if self.next:
...
class UserFollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        relation = Relation.objects.filter(from_user=request.user , to_user=user)
        if relation.exists():
            messages.error(request, 'kosoo ino qbln gaeedi.', 'danger')
        else:
            Relation(from_user=request.user, to_user=user).save()
            messages.success(request, 'mrc haji.', 'success')
...
class UserUnfollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            relation.delete()
            messages.success(request, 'az sharesh khalas shodi.', 'success')
        else:
            messages.error(request, 'kosmaqz follow nkrdi.', 'danger')
        return redirect('account:user_profile', user.id)
